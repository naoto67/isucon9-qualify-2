...
func postInitialize(w http.ResponseWriter, r *http.Request) {
	...
	err = dbx.Select(&mCategories, "SELECT * FROM categories")
	if err != nil {
		log.Print(err)
		outputErrorMsg(w, http.StatusInternalServerError, "db error")
		return
	}
	categoryMap = make(map[int]Category, len(mCategories))
	for i := range mCategories {
		categoryMap[mCategories[i].ID] = mCategories[i]
	}
...
func getNewItems(w http.ResponseWriter, r *http.Request) {
	...
	if itemID > 0 && createdAt > 0 {
		...
		// 1st page
		err := dbx.Select(&items,
			"SELECT * FROM `items` WHERE `status` IN (?,?) ORDER BY `price` DESC, `created_at` DESC, `id` DESC LIMIT ?",
			ItemStatusOnSale,
			ItemStatusSoldOut,
			ItemsPerPage+1,
		)
...
func getTransactions(w http.ResponseWriter, r *http.Request) {
	...
	if itemID > 0 && createdAt > 0 {
		...
		// 1st page
		err := tx.Select(&items,
			"(SELECT * FROM `items` WHERE `seller_id` = ? ORDER BY `created_at` DESC, `id` DESC) UNION ALL ( SELECT * FROM `items` WHERE `buyer_id` = ? ORDER BY `created_at` DESC, `id` DESC) ORDER BY created_at DESC, id DESC LIMIT ?",
			user.ID,
			user.ID,
			TransactionsPerPage+1,
		)
